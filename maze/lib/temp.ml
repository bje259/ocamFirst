...
let rec init_tree n f a b =
  ...
  if n = 0 then
    ...
    let l = n - 1 - r in
    Node
      (init_tree l f (2 * a) (a + b), f b, init_tree r f (2 * a) ((2 * a) + b))

let init n f = { size = n; tree = init_tree n f 1 0 }
...
let rec set_tree t i v =
  ...
  | Node (l, x, r) ->
      if i = 0 then
        ...
        Node (set_tree l (i / 2) v, x, r)
      else
        Node (l, x, set_tree r ((i / 2) - 1) v)
...
let cons v a = { size = a.size + 1; tree = cons_aux v a.tree }
...
(* high extension *)
let rec snoc_aux s t v =
  match t with
  | Empty -> Node (Empty, v, Empty)
  | Node (l, x, r) ->
      if s mod 2 = 1 then
        Node (snoc_aux (s / 2) l v, x, r)
      else
        Node (l, x, snoc_aux ((s / 2) - 1) r v)

let snoc a v = { size = a.size + 1; tree = snoc_aux a.size a.tree v }
...
let rec liat_aux s = function
  ...
  | Node (l, x, r) ->
      if s mod 2 = 0 then
        Node (liat_aux (s / 2) l, x, r)
      else
        Node (l, x, liat_aux (s / 2) r)
...
let mapi f a =
  let rec map a b = function
    | Empty -> Empty
    | Node (l, x, r) ->
        Node (map (2 * a) (a + b) l, f b x, map (2 * a) ((2 * a) + b) r)
  in
...
let foldi f acc a =
  ...
  let rec loop acc i current ((left, right) as next) =
    if i = a.size then (
      assert (
        Queue.is_empty current && Queue.is_empty left && Queue.is_empty right);
      acc)
    else if Queue.is_empty current then (
...
let iteri f a = foldi (fun () i x -> f i x) () a
let iter f a = foldi (fun () _ x -> f x) () a
let fold f acc a = foldi (fun acc _ x -> f acc x) acc a
